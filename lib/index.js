...
module.exports = function (repo, options) {
  ...
  return function (context) {
    ...
    if (isNewUpdate(newestVersion, pluginVersion)) {
      ...
      var accessory = NSView.alloc().initWithFrame(NSMakeRect(0, 0, 350, 200))

      // var title = NSTextView.alloc().initWithFrame(NSMakeRect(0, 225, 350, 20))
      // title.string = 'Version ' + newestVersion + ' is available (current version: ' + pluginVersion + ')'
      // title.drawsBackground = false
      // title.editable = false

      var scroll = NSScrollView.alloc().initWithFrame(NSMakeRect(0, 0, 350, 200))
      ...
      alert.setMessageText('Version ' + newestVersion + ' is available (current version: ' + pluginVersion + ')')
